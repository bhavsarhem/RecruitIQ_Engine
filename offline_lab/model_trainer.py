...
def train_lgbm_ranker(
    ...
    # Fit a min-max scaler on the training set predictions
    # ----------------------------------------------------------------
    from sklearn.preprocessing import MinMaxScaler
    raw_preds = model.predict(X)
    scaler = MinMaxScaler()
    scaler.fit(raw_preds.reshape(-1, 1))

    # Wrap model + scaler into a bundle
    model_bundle = {
        "model": model,
        "scaler": scaler,
...
# Update ranker.py's LGBM loading to use the bundle format
UPDATED_LGBM_LOADER = """
# In ranker.py, update _try_load_lgbm() to handle the bundle:
def _try_load_lgbm():
    ...
    try:
        ...
        _lgbm_model = bundle  # store the whole bundle
...
def main():
    ...
    parser.add_argument("--features", default="offline_lab/artifacts/features.npz")
    parser.add_argument("--labels", default="offline_lab/artifacts/labels.json")
    parser.add_argument("--out", default="offline_lab/artifacts/lgbm_ranker.pkl")

...
_LGBM_MODEL_PATH = _ARTIFACTS_DIR / "lgbm_ranker.pkl"
...
def _try_load_lgbm():
    ...
    try:
        ...
        # Support both plain model and bundle dict formats
        if isinstance(bundle, dict) and "model" in bundle:
            _lgbm_model = bundle  # full bundle: {model, scaler, feature_names}
            _feature_names = bundle["feature_names"]
        else:
            _lgbm_model = bundle  # plain model
...
def _score_candidate(candidate: dict, features: dict[str, float]) -> float:
    """
    Returns a score in [0, 1] for the candidate.
    ...
    if _lgbm_model is not None and _feature_names is not None:
        try:
            ...
            # Handle both bundle dict and plain model
            if isinstance(_lgbm_model, dict):
                lgbm = _lgbm_model["model"]
                raw = float(lgbm.predict(feat_row)[0])
            else:
                raw = float(_lgbm_model.predict(feat_row)[0])

            # Use raw score without scaler compression for better spread.
            # Raw LambdaRank scores can be any float; clip to [0,1] after
            # applying a simple linear rescaling anchored at 0.
            # We'll normalize in the post-processing step.
